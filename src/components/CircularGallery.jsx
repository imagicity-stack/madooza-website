...
const loadOgl = (() => {
  ...
  return () => {
    if (typeof window === 'undefined') {
      return Promise.reject(new Error('OGL can only be loaded in the browser.'));
    }
    if (window.OGL) {
      return Promise.resolve(window.OGL);
    }
    if (!promise) {
      promise = new Promise((resolve, reject) => {
        const existing = document.querySelector(`script[src="${OGL_URL}"]`);
        if (existing) {
          existing.addEventListener('load', () => resolve(window.OGL));
          existing.addEventListener('error', () => reject(new Error('Failed to load OGL.')));
          return;
        }
        const script = document.createElement('script');
        script.src = OGL_URL;
        script.async = true;
        script.onload = () => resolve(window.OGL);
...
function createTextTexture(gl, TextureClass, text, font = 'bold 30px monospace', color = 'black') {
  ...
  const texture = new TextureClass(gl, { generateMipmaps: false });
  texture.image = canvas;
  return { texture, width: canvas.width, height: canvas.height };
}

class Title {
  constructor({ ogl, gl, plane, renderer, text, textColor = '#545050', font = '30px sans-serif' }) {
    autoBind(this);
    this.ogl = ogl;
    this.gl = gl;
    this.plane = plane;
